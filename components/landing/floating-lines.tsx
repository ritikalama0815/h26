...
export default function FloatingLines({
  ...
  const getLC = (w: 'top' | 'middle' | 'bottom') => {
    if (typeof lineCount === 'number') return lineCount;
    if (!enabledWaves.includes(w)) return 0;
    return lineCount[enabledWaves.indexOf(w)] ?? 6;
  };
  const getLD = (w: 'top' | 'middle' | 'bottom') => {
    if (typeof lineDistance === 'number') return lineDistance;
    if (!enabledWaves.includes(w)) return 0.1;
    return lineDistance[enabledWaves.indexOf(w)] ?? 0.1;
  };
